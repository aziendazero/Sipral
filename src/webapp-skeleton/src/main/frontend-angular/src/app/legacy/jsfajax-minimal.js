...
/*richfaces\release-3.3.4\framework\impl\src\main\javascript\ajaxjsf\sarissa.js*/
...
/** @private */
Sarissa._SARISSA_IS_IE = (navigator.userAgent.toLowerCase().indexOf("msie") > -1 || navigator.userAgent.toLowerCase().indexOf("trident") > -1) && navigator.userAgent.toLowerCase().indexOf("opera") == -1;
/** @private IE9, IE10 or IE11 (also correct identification of compatibility mode/view) */
Sarissa._SARISSA_IS_IE9 = Sarissa._SARISSA_IS_IE && (navigator.userAgent.toLowerCase().indexOf("msie 9") > -1 || navigator.userAgent.toLowerCase().indexOf("msie 10") > -1 || navigator.userAgent.toLowerCase().indexOf("trident/7.0") > -1 || document.documentMode >= 9);
...
if(Sarissa._SARISSA_IS_IE){
     ...
     * @constructor
     */
    //Avoid overriding IE's default XMLHttpRequest object with an old MSXML ActiveX -> fixes bug in upload by FormData()
    var zXMLHttpRequest = function() {
     ...
     */
    zXSLTProcessor.prototype.importStylesheet = function(xslDoc){
     ...
     */
    zXSLTProcessor.prototype.transformToDocument = function(sourceDoc){
        // fix for bug 1549749
        var outDoc;
        if(_SARISSA_THREADEDDOM_PROGID){
     ...
     * @return {DOMDocument} The transformation result as a DOM Document
     */
    zXSLTProcessor.prototype.transformToFragment = function (sourceDoc, ownerDoc) {
        this.processor.input = sourceDoc;
        this.processor.transform();
     ...
     */
    zXSLTProcessor.prototype.setParameter = function(nsURI, name, value){
     ...
     */
    zXSLTProcessor.prototype.getParameter = function(nsURI, name){
        nsURI = "" + (nsURI || "");
     ...
     */
    zXSLTProcessor.prototype.clearParameters = function(){
    ...
    //To avoid overriding IE's default XMLHttpRequest object, use zXMLHttpRequest also in other browsers
    var zXMLHttpRequest = XMLHttpRequest;
    ...
    if(Sarissa._SARISSA_HAS_DOM_CREATE_DOCUMENT){
         ...
         * @private
         */
        var _sarissa_XMLDocument_onload = function(){
...
if(!window.DOMParser || Sarissa._SARISSA_IS_IE9){
    if(Sarissa._SARISSA_IS_SAFARI){
        ...
        //Avoid overriding IE's default DOMParser object
        //It works also in IE8!
//        DOMParser = function() { };
//        DOMParser.prototype.parseFromString = function(sXml, contentType){
...
if(!window.XMLSerializer && Sarissa.getDomDocument && Sarissa.getDomDocument("","foo", null).xml){
    ...
    var IE9XMLSerializer= XMLSerializer;
...
/*richfaces\release-3.3.4\framework\impl\src\main\javascript\ajaxjsf\log4ajax.js*/
...
LOG._logToServer = function(msg, logLevel)
{
    ...
    //Added log to PHI runtime
    msg = 'Javascript ' + msg;

    logError(msg, window.Phi.cid);
}
...
if(!window.A4J){
    window.A4J={}
    /*richfaces\release-3.3.4\framework\impl\src\main\javascript\ajaxjsf\JSFAJAX.js*/
// Global Variables
// var timeout = null;
...
// Modified by Alexander J. Smirnov to use as JSF AJAX-like components.
    ...
    A4J.AJAX.XMLHttpRequest.prototype = {
         ...
         */
        send : function(){
            //Use zXMLHttpRequest instead of XMLHttpRequest to avoid polluting global space -> fixes bug in upload by FormData()
            this._request = new zXMLHttpRequest();
            if (this._request.withCredentials === false) {
                ...
                if (_this._request.readyState == 4  ) {
                    ...
                    if(requestStatus == 200){
                        try {
                            ...
                            //LOG._logToServer('>>>\n' + navigator.userAgent + "\n" + _this.getResponseText(), LOG.DEBUG);
                        } catch(e) {
...
//		LOG.debug("getElementsByTagNameNS found "+elements.length);
        ...
        getElementById : function(id){
            ...
            if(oDoc){
                ...
                else /*if(typeof(oDoc.selectSingleNode) != "undefined")*/ { // Removed check because if compiled with ng build --prod exploodes
        ...
        _evaluateScript: function(node) {
            ...
            try {
                ...
                LOG.fatal("ERROR Evaluate script: " + e.message + "\n" + newscript);
            }
        },
        ...
        updatePagePart : function(id, isLast){
            ...
            if ( oldnode  ) {
                ...
                if(!window.opera && !A4J.AJAX.isWebkitBreakingAmps() && oldnode.outerHTML && !oldnode.tagName.match( /(tbody|thead|tfoot|tr|th|td)/i ) ){
                    ...
                    //LOG._logToServer(id + ' updatePagePart >>>' + navigator.userAgent + "\n" + oldnode.outerHTML + '\n\n\n', LOG.DEBUG);
                } else {
                    // need to check for firstChild due to opera 8 bug with hasChildNodes
                    Sarissa.clearChildNodes(oldnode);
...
//If request for same queue already performed, replace with current parameters.
    A4J.AJAX.Submit =  function(formId, event , options ) {
        ...
        //Added by AZ to prevent navigation
    ...
    A4J.AJAX.processResponse = function(req) {
        ...
        if( ajaxResponse != "true"){
            ...
            if(req.getParserStatus() == Sarissa.PARSED_OK){
                ...
                if(!Sarissa._SARISSA_IS_IE9){ //FIX IE BY AZ
...
// Class for build query string.
    A4J.Query = function(containerId, form){
        ...
        this._actionUrl = ( this._form.action)?this._form.action:this._form; // BUG TypeError: Cannot read property 'action' of null Chrome/49.0.2623.112
    };
    ...
    A4J.Query.prototype = {
        ...
        // Added support for HTML5 input type date, datetime-local and time
        date : function(control){
          this._value_query(control);
        },
        'datetime-local': function(control){
          this._value_query(control);
        },
        time : function(control){
          this._value_query(control);
        },
        // END Added support for HTML5 input type date, datetime-local and time
...
// Return action URL ( append extention, if present )
        getActionUrl : function( ext ) {
            ...
            if(window.location.port == '3000') { //developement localehost:3000 -> cors -> localhost:8080
                actionUrl = environment.apiUrl + 'home-jsf.seam';
            }
...
//Test for re-evaluate Scripts in updated part. Opera & Safari do it.
    ...
    /*richfaces\release-3.3.4\framework\impl\src\main\javascript\ajaxjsf\queue.js*/
...
//queue constructor
    ...
    /*richfaces\release-3.3.4\framework\impl\src\main\resources\org\ajax4jsf\javascript\scripts\form.js*/
    ...
    if(!A4J.findForm){
        ...
        // Added for webpack build
        window._clearJSFFormParameters = _clearJSFFormParameters;
