...
public class NestedSetResultTransformer<N extends NestedSetNode> implements ResultTransformer {
    ...
    private static final Logger log = Logger.getLogger(NestedSetResultTransformer.class);
    ...
    public Object transformTuple(Object[] objects, String[] aliases) {
        ...
        Long nestedSetNodeParentId = nestedSetNode.getParentNodeInfo().getOwner().getInternalId(); // Store the parent id before making a duplicate
        ...
        if (!nestedSetNodeParentId.equals(currentParent.getWrappedNode().getInternalId())) {
    ...
    private NestedSetNodeWrapper<N> findParentInTree(Serializable parentId, NestedSetNodeWrapper<N> startNode) {
        if (!parentId.equals(startNode.getWrappedNode().getInternalId()) && startNode.getWrappedParent() != null) {
            return findParentInTree(parentId, startNode.getWrappedParent());
        } else if (parentId.equals(startNode.getWrappedNode().getInternalId())) {
            return startNode;
        } else {
            return null;
        }
