...
public class PressionBarValidator implements Validator {

	//only digits syntax
	//private static final String regex = "\\d{1,5}(.\\d{1,2})";
	...
	public void validate(FacesContext facesContext, UIComponent uiComponent, Object value) throws ValidatorException {
		...
		*/
		if (parts!=null){
			if ((parts.length!=2) || !isInteger(parts[0]) || !isInteger(parts[1]) || parts[0].length()<1 || parts[0].length()>5 || parts[1].length()<1 || parts[1].length()>2)
