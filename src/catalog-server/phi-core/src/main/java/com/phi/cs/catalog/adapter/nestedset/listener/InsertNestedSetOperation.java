...
class InsertNestedSetOperation extends NestedSetOperation {
    ...
    private static final Logger log = Logger.getLogger(InsertNestedSetOperation.class);
    ...
    protected void beforeExecution() {
        ...
        if (node.getParentNodeInfo() == null) {
            ...
            parentThread = node.getInternalId();
            newLeft = 1l;
            newRight = 2l;
        } else {
    ...
    protected void executeOnDatabase(EventSource ss) {
        ...
        Query updateNode =
                ss.createQuery("update " + nodeEntityName + " n set " +
                               ...
                               " where n.internalId = :id");
         
//      Query updateNode = ss.getNamedQuery("updateNestedSet."+node.getClass().getSimpleName()); 
        updateNode.setParameter("thread", parentThread);
        updateNode.setParameter("left", newLeft);
        updateNode.setParameter("right", newRight );
        updateNode.setParameter("id", node.getInternalId());
